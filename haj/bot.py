...
class Bot:
    ...
    async def on_guild_join(self, guild: discord.Guild):
        haj.utils.update_guild_ids(self.client, self.database)

    async def on_message(self, message: discord.Message):
        if message.author != self.client.user:
            haj.utils.update_guild_ids(self.client, message.channel)
            if message.content[0] == haj.utils.get_command_prefix(self.client, message.channel):
                args = shlex.split(message.content[1:])
                args[0] = args[0].lower()
                if (haj.utils.is_admin(self.database, message.author.id) and
                        isinstance(message.channel, discord.DMChannel) and args[0] in haj.commands.admin_commands):
                    await haj.commands.get(haj.commands.admin_commands[args[0]])(message, self.client, args)
                elif args[0] in haj.commands.commands:
                    await haj.commands.get(haj.commands.commands[args[0]])(message, self.client, args)
                else:
